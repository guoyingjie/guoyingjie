...
//默认选中第一个场所值
function copyValue(){
	$(".siteName1").text($('.siteList > li').eq(0).text());
	if($(".balance span").text()==0){
		$('.balance').addClass('on');
	}else{
		$('.balance').removeClass('on');
	}	
...
function goPay(){
		...
		priceNum+"&siteId="+siteId+"&addMealNum="+addMealNum+"&addMealUnit="+addMealUnit+"&mealType="+mealType+"&openid="+openid+"&userName="+username+"&payWay="+payWay+"&times="+$("#times").val()+"&v="+v;
...
function goBalancePay(){
	...
	if(amount<0){
		...
		$(".yes").click(function(){
			window.location.href= basePath+"/weChatPayOther/weixinPayOtherSite?nums="+num+"&amount="+amount+"&siteConfigId="+siteConfigId+
			"&priceNum="+priceNum+"&siteId="+siteId+"&addMealNum="+addMealNum+"&addMealUnit="+addMealUnit+"&mealType="+mealType+"&userAccount="+userAccount+
			"&usedAccount="+balance+"&openid="+openid+"&userName="+username+"&payWay="+payWay+"&times="+$("#times").val()+"&v="+v;
...
function getPayMessage(){
	var openid = $("#username").val();
	var siteId=$("#siteId").val();
	$.ajax({
		type:"post",
		url:basePath+"/weChatPublicNum/payRetroactiveMeal",
		data:{
			userName:openid,
			siteId:siteId,
			num:num
		...
		success:function(data){
			eval("data="+data);
			if(data.code==200){
				$(".siteName1").text(data.data.siteInof.site_name);
				$(".balance span").text(data.msg);
				var htmls="<ul>";
				for (var i = 0; i <data.data.list.length; i++) {
					if(data.data.list[i].price_type<=3){
						htmls+=
							"<li class='tLi' data-text="+"'"+data.data.list[i].describe+"'" +" siteId="+data.data.list[i].site_id +" mealNum="+data.data.list[i].giveMeal+
							" mealType="+data.data.list[i].giveMealUnit+" value="+data.data.list[i].id+
							" reommend="+data.data.list[i].recommendState+" charge_type="+data.data.list[i].charge_type+
							" prices="+data.data.list[i].unit_price+" price_type="+data.data.list[i].price_type+
							" price_num="+data.data.list[i].price_num+">"+data.data.list[i].name+"</li>"
					}else{
						htmls+=
						"<li class='fLi' data-text="+"'"+data.data.list[i].describe+"'"+" siteId="+data.data.list[i].site_id +" mealNum="+data.data.list[i].giveMeal+
						" mealType="+data.data.list[i].giveMealUnit+" value="+data.data.list[i].id+
						" reommend="+data.data.list[i].recommendState+" charge_type="+data.data.list[i].charge_type+
						" prices="+data.data.list[i].unit_price+" price_type="+data.data.list[i].price_type+
						" price_num="+data.data.list[i].price_num+">"+data.data.list[i].name+"</li>"
