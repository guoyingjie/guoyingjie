...
<p class="balance">可用余额：<span>10</span>元</p>
...
<div class="maskSite">
	<div class="content">
		<p class="tc"><span>关闭</span></p>
		<ul class="siteList">
			<c:forEach var="p" items="${siteAll}">
				<li value="${p.key.id }">${p.key.site_name }</li>
			</c:forEach>
		</ul>
	</div>
</div>
...
<script type="text/javascript" src="${mobilePath1}/js/pay.js?<%=getTimestamp%>"></script>
</body>
</html>
