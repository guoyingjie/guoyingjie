...
<div class="log-mask">
		...
		<div class="warn">
			...
			<div class="buttons" style="margin:0 auto;margin-top: 10px;">
				<button id="sure" style="display: block; margin: 0 auto; float: none;">我知道了</button>
			</div>
		</div>
	</div>
	...
	if(chillireturn !='null' && chillireturn.length>1){
		...
		if(data.code == 201){
			...
			if(model!=0){
				...
				obj.open("该账号已登录！请使用当前登录设备<span>"+MAC+"</span>的浏览器访问!<br><span>http://logout/</span>退出登录后，再在新设备上登录");
			}
			$("#sure").click(function(){
			...
			var model=data.termial;
			var nasipaddress = data.uamip;
			var urllogoff = "http://"+nasipaddress+"/logout";
			if(model!=0){
			...
			$("#cancel").click(function(){
				...
				$(".log-mask").hide();
					...
					if(model!=0){
						...
						obj.open("该账号已登录！请使用当前登录设备<span>"+MAC+"</span>的浏览器访问!<br><span>http://1.0.0.0/logout</span>退出登录后，再在新设备上登录");
					}
					$("#sure").click(function(){
						// window.location.href=urllogoff;
						$(".log-mask").hide();
