...
public class TDESUtil {
	...
	private static Logger logger = Logger.getLogger(TDESUtil.class);
    ...
    public static String encrypt2HexStr(byte[] keys, String sourceData) {
        ...
        try {
            ...
            logger.info("原数据据:" + sourceData);
            logger.info("原数据byte长度:" + merchantData);
            logger.info("原数据HEX表示:" + bytes2Hex(source));
            //2.计算补位
           ...
           // System.out.println("需要补位 :" + y);
            ...
            logger.info("补位后的byte数组长度:" + resultByte.length);
            logger.info("补位后数据HEX表示:" + bytes2Hex(resultByte));
            logger.info("秘钥HEX表示:" +bytes2Hex(keys));
            logger.info("秘钥长度:" +keys.length);

            byte[] desdata = TDESUtil.encrypt(keys, resultByte);
            logger.info("加密后的长度:" + desdata.length);
